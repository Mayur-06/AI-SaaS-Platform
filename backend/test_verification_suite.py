...
def test_1_rate_limiting():
    ...
    print(f"Free plan rate limit: {free_plan.requests_per_minute} req/min")

    # Create test org & user on Free plan
    unique = uuid.uuid4().hex[:8]
    org = Organization.objects.create(name=f"RL Test Org {unique}", slug=f"rl-org-{unique}", plan=free_plan)
    ...
    api_key_obj = APIKey.objects.create(organization=org, name=f"RL Key {unique}")
    ...
    results = []
    print(f"Sending 13 rapid requests with 10/min limit...")
    for i in range(1, 14):
        # Hit /api/keys/ (which runs rate limit middleware)
    ...
    # Verify requests 1..10 succeeded (status 200)
    for i in range(10):
        req_num, status, limit, rem, retry = results[i]
        assert status == 200, f"Request {req_num} expected 200, got {status}"
