...
if count >= limit then
    return {limit, count, math.floor(reset_time), 0}
end

redis.call('ZADD', key, now, now .. '-' .. math.random())
redis.call('EXPIRE', key, math.floor(ttl))
return {limit, count + 1, math.floor(reset_time), 1}
"""
...
def check_rate_limit(org_id, api_key_id, limit):
    ...
    if api_key_id:
        key = f"ratelimit:{org_id}:{api_key_id}"
    else:
        key = f"ratelimit:{org_id}"

    script = get_rate_limit_script()
    result = client.evalsha(script, 1, key, now, window, limit)
    effective_limit = result[0]
    count = result[1]
    reset_time = result[2]
    allowed = (result[3] == 1) if len(result) > 3 else (count < effective_limit)
    remaining = max(0, effective_limit - count) if allowed else 0
