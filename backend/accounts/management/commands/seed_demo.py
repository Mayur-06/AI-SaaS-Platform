    ...
    def handle(self, *args, **options):
        with transaction.atomic():
            ...
            for plan, primary, fallbacks in [
                (free_plan, gemini_flash_model, []),
                (pro_plan, gemini_flash_model, [gemini_pro_model]),
                (enterprise_plan, gemini_pro_model, [gemini_flash_model]),
            ]:
                rule = RoutingRule.objects.filter(plan=plan).first()
                if not rule:
                    rule = RoutingRule.objects.create(plan=plan, primary_model=primary, timeout_seconds=10)
                else:
                    rule.primary_model = primary
                    rule.save()
